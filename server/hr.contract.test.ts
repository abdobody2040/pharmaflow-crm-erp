...
describe("ERP/HR procedure contracts", () => {
  ...
  it("does not allow a rep to invoke restricted payroll exports", async () => { const caller = appRouter.createCaller({ user: rep, req: { headers: {} } as any, res: {} as any }); await expect(caller.hr.payroll.export({ periodStart: new Date("2026-08-01"), periodEnd: new Date("2026-08-31"), format: "csv" })).rejects.toMatchObject({ code: "FORBIDDEN" }); });
});
