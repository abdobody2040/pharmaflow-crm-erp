...
أثبتت جولة QA المحلية أن مسارات الأعمال والعقود والواجهات التشغيلية تستمر في العمل بعد ترقيات التبعيات ومعالجات الأمان. أضيف اختبار Playwright يزور كل المسارات التشغيلية المسجلة بمستخدم إداري وعقود محلية مقلدة، مع رصد أخطاء الصفحة غير المعالجة. كما حُسّنت سلسلة التبعيات وأزيلت كل الثغرات العالية التي كان يعيدها تدقيق تبعيات الإنتاج.
...
| E2E محلي                     |  ✅ ضمن بيئة mock   | `pnpm exec playwright test`؛ يشمل الدخول وRTL وGPS mock والمسارات التشغيلية  |
...
| Browser مصادق ببيانات فعلية | fixture موجود لكنه لا يعمل بلا بيانات disposable                              | ⚠️ staging/fixture مطلوب |
...
يبقى تنبيه **moderate** واحد في `uuid@8.3.2` كتبعـية داخل `exceljs@4.4.0`؛ الإصدار المصحح الذي يعرضه التدقيق ليس ترقية متوافقة مؤكدة مع النسخة الحالية من ExcelJS. لا يُستخدم هذا المسار لمعالجة XLSX وارد من المستخدم، ومع ذلك يجب إعادة تقييمه عند إصدار ExcelJS متوافق أو قبل اعتماد إنتاجي منظم.
...
| حراسة webhook                  | اختبار يرفض HTTP وlocalhost وعناوين RFC1918 وcredentials المضمّنة قبل الشبكة أو DB                              |
...
يتطلب الإغلاق التشغيلي الحقيقي بيئة staging مع MySQL وبيانات disposable: DAST مصادق، اختبار مصفوفة الأدوار من المتصفح، rate limiting عبر عدة مثيلات، اختبار reverse proxy وTLS حقيقي، benchmark وEXPLAIN، وNTP/backup-restore. كما يلزم تنفيذ Android وiOS على أجهزة فعلية لمحاكاة أذونات GPS والعمل بالخلفية والمزامنة offline→online. لا يدّعي هذا التقرير تنفيذ أي من هذه البنود.
